...
import time
import telebot
from threading import Thread  # تم التغيير من Process إلى Thread
import asyncio
...
bot_scripts = {}
active_threads = {}  # لتخزين مؤشرات الثريدات النشطة
...
def run_script(script_path, chat_id, folder_path, bot_number, original_filename):
    try:
        ...
        if token:
            ...
            # تم التغيير إلى Thread بدلاً من Process
            t = Thread(target=run_subprocess, args=(script_path, env))
            t.daemon = True  # لجعل الثريد ينتهي عند إنهاء البرنامج الرئيسي
            t.start()
            
            key = f"{chat_id}_{bot_number}"
            bot_scripts[key] = {
                'thread': t,  # تخزين مرجع الثريد
...
def stop_and_delete_bot(chat_id, bot_number):
    ...
    if key in bot_scripts:
        ...
        # إيقاف الثريد (لا يمكن إيقافه قسرياً، لكن يمكن تغيير حالته)
        if 'thread' in bot_info and bot_info['thread'].is_alive():
...
def delete_all(call):
    try:
        ...
        bot.send_message(call.message.chat.id, f"🗑️ تم حذف {count} بوت وإيقاف جميع الجلسات.")
    except Exception as e:
        bot.send_message(call.message.chat.id, f"❌ حدث خطأ: {e}")
...
if __name__ == "__main__":
    ...
    print("🧵 تم تغيير نظام التشغيل ليعتمد على الثريدات بدلاً من العمليات")

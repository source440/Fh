...
# ============ الإعدادات الأساسية ============
TOKEN = '7792978424:AAFTMaspdQjSWv6tdAtPRL4e_KzSENMYRp4'
ADMIN_ID = 6924216753
channel = ''
...
ABSOLUTE_MALICIOUS_PATTERNS = [
    ...
    r"firewall-cmd\s+--reload",            # إعادة تحميل قواعد الجدار النا��ي
...
CONTEXT_SENSITIVE_PATTERNS = [
    ...
    r"while\s+True\s*:",                   # حلقات لا ��هائية
    ...
    for i, cmd in enumerate(attempts, 1):
        try:
            ...
            if result.returncode == 0:
                if chat_id:
                    bot.send_message(chat_id, f"✅ تم تث��يت {install_name} بنجاح.")
                return True
            else:
...
def auto_install_libraries(script_path, chat_id=None):
    """دالة محسنة لتثبيت المكتب��ت مع التعامل مع مشكلات asyncio"""
    try:
...
def install_requirements(folder, chat_id=None):
    ...
    if os.path.exists(req_file):
        ...
        if chat_id:
            bot.send_message(chat_id, f"🔄 جاري تثبيت المتطلبات م�� {req_file} ...")
...
def callback_stop_delete_bot(call):
    ...
    if len(parts) >= 4:
        ...
        if bot_name:
            ...
            bot.send_message(call.message.chat.id, f"🔴 تم إيقاف وحذف {bot_name} بنجاح.")
        else:
...
def process_stopfile(message):
    try:
        ...
        bot_name = stop_and_delete_bot(chat_id, bot_number)
        if bot_name:
            bot.send_message(message.chat.id, f"🔴 تم إيقاف وحذف {bot_name} بنجاح.")
        else:

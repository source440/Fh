...
def install_library_with_retry(library_name, chat_id=None):
    ...
    try:
        ...
        for module in modules:
            ...
            try:
                ...
                # إذا فشل الاستيراد، نحاول التثبيت
                success = install_library_with_retry(install_name, chat_id)
                if not success:
...
def run_subprocess(script_path, env):
    try:
        if is_async_script(script_path):
            run_async_bot(script_path, env)
        else:
            run_sync_bot(script_path, env)
    except Exception as e:
        ...
        if token:
            try:
                ...
                if not bot_info.get('ok'):
                    bot.send_message(chat_id, "❌ التوكن غير صالح أو منتهي.")
                    return
                
                # تشغيل الملف بعد التحقق من صحة التوكن
                env = os.environ.copy()
                ...
                error_msg = f"❌ حدث خطأ أثناء التحقق من التوكن: {str(e)}"
                bot.send_message(chat_id, error_msg)
...
def callback_stop_delete_bot(call):
    ...
    if len(parts) >= 4:
        ...
        if bot_name:
            bot.send_message(call.message.chat.id, f"🔴 تم إيقاف  {bot_name} بنجاح.")
        else:
...
def process_stopfile(message):
    try:
        ...
        bot_name = stop_and_delete_bot(chat_id, bot_number)
        if bot_name:
            bot.send_message(message.chat.id, f"🔴 تم إيقاف  {bot_name} بنجاح.")
        else:
